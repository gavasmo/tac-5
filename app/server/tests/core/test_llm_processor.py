...
class TestGenerateRandomQuery:
    ...
    def test_generate_random_query_with_anthropic_success(self, mock_anthropic_class):
        ...
        mock_response = MagicMock()
        mock_response.content[0].text = "What is the average price of products in stock?"
        mock_client.messages.create.return_value = mock_response
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'test-key'}):
            ...
            assert result == "What is the average price of products in stock?"
            mock_client.messages.create.assert_called_once()
    ...
    def test_generate_random_query_with_anthropic_clean_markdown(self, mock_anthropic_class):
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'test-key'}):
            schema_info = {'tables': {'orders': {'columns': {}, 'row_count': 5}}}

            result = generate_random_query_with_anthropic(schema_info)

            assert result == "How many orders were placed last week?"
    ...
    def test_clean_generated_query_text_strips_quotes_and_fences(self):
        assert clean_generated_query_text('```\n"Hello?"\n```') == "Hello?"
        assert clean_generated_query_text("'Hello?'") == "Hello?"
        assert clean_generated_query_text("Hello?") == "Hello?"

    def test_generate_random_query_no_tables(self):
        schema_info = {'tables': {}}
    ...
    def test_generate_random_query_anthropic_fallback(self, mock_anthropic_func):
        mock_anthropic_func.return_value = "Which products are low in stock?"
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'anthropic-key'}, clear=True):
            ...
            assert result == "Which products are low in stock?"
    ...
    def test_generate_random_query_no_api_keys(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert "No LLM provider API key configured" in str(exc_info.value)